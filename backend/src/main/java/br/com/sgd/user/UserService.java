...
public class UserService {
  ...
  public User create(String nome, String email, String password, Set<Role> perfis) {
    ...
    User user = users.save(new User(nome, email, passwords.encode(password), perfis));
    audit.save(new AuditLog(user, "USUARIO", "CRIACAO", "{}"));
    return user;
  }

  public User update(long id, String nome, Set<Role> perfis, Boolean ativo) {
    User user = users.findById(id).orElseThrow(UserNotFoundException::new);
    boolean deactivate = Boolean.FALSE.equals(ativo) && user.isAtivo();
    user.update(nome, perfis, ativo);
    if (deactivate) refreshTokens.revokeAllByUserId(user.getId(), Instant.now());
    audit.save(new AuditLog(user, "USUARIO", "ATUALIZACAO", "{}"));
    return user;
  }
