...
public class SecurityConfig {
  ...
  @Bean
  ApplicationRunner adminBootstrap(AuthService authService) {
    return arguments -> authService.bootstrapAdmin();
  }

  @Bean
  SecurityFilterChain securityFilterChain(
      HttpSecurity http, JwtAuthenticationFilter jwtFilter, ObjectMapper json) throws Exception {
                    ...
                    .authenticationEntryPoint(
                        (request, response, exception) ->
                            writeProblem(
                                ...
                                "Não autorizado",
                                "A autenticação é obrigatória ou expirou."))
                    .accessDeniedHandler(
                        (request, response, exception) ->
                            writeProblem(
                                response,
  ...
  private static void writeProblem(
      HttpServletResponse response, ObjectMapper json, int status, String title, String detail)
      throws IOException {
      ...
      if (header != null && header.startsWith("Bearer ")) {
        ...
        if (jwt.isValid(token))
          users
              ...
              .filter(br.com.sgd.user.User::isAtivo)
              .ifPresent(
                  user -> {
                    var authorities =
