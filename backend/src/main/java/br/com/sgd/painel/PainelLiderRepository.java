...
public interface PainelLiderRepository extends Repository<Encontro, Long> {
  ...
  @Query(
      value =
          "select count(*) from encontros where discipulado_id=:discipuladoId and situacao='REALIZADO' and data between :inicio and :fim",
      nativeQuery = true)
  ...
  @Query(
      value =
          "select distinct a.id as adolescenteId, a.nome as nome from adolescentes a "
              ...
              + "and e.data between :inicio and :fim order by nome, adolescenteId",
      nativeQuery = true)
