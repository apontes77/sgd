...
public class PainelLiderService {
  ...
  public PainelLiderResponse consultar(User usuario, LocalDate inicio, LocalDate fim) {
    ...
    painel
        .frequenciasMensais(discipuladoId, inicio, fim)
        .forEach(
            item ->
                meses.put(
                    item.getReferencia(),
    ...
    painel
        .visitantesMensais(discipuladoId, inicio, fim)
        .forEach(
            item ->
                meses.compute(
                    item.getReferencia(),
    ...
    List<EvolucaoMensal> evolucao =
        meses.values().stream().sorted(Comparator.comparing(EvolucaoMensal::referencia)).toList();
    ...
    Resumo resumo =
        new Resumo(
            painel.encontrosRealizados(discipuladoId, inicio, fim),
            presentes,
            ausentes,
            visitantes,
            percentual(presentes, ausentes));
    DiscipuladoInfo info =
        new DiscipuladoInfo(
            discipuladoId,
            discipulado.getNome(),
            discipulado.getSexo().name(),
            discipulado.isAtivo());
    return new PainelLiderResponse(
        inicio, fim, info, resumo, evolucao, montarDiscipulos(discipuladoId, inicio, fim));
  }
  ...
  private List<Discipulo> montarDiscipulos(long discipuladoId, LocalDate inicio, LocalDate fim) {
    ...
    painel
        .discipulosNoPeriodo(discipuladoId, inicio, fim)
        .forEach(
            item ->
                resultado.put(
                    item.getAdolescenteId(),
                    new DiscipuloAcumulado(item.getAdolescenteId(), item.getNome())));
    painel
        .frequenciasIndividuaisMensais(discipuladoId, inicio, fim)
        .forEach(
            item -> {
              DiscipuloAcumulado discipulo =
                  resultado.computeIfAbsent(
                      item.getAdolescenteId(), id -> new DiscipuloAcumulado(id, item.getNome()));
              ...
              discipulo.evolucao.add(
                  new EvolucaoIndividual(
                      item.getReferencia(), presentes, ausentes, percentual(presentes, ausentes)));
            });
    return resultado.values().stream()
        .sorted(
            Comparator.comparing(DiscipuloAcumulado::nome, String.CASE_INSENSITIVE_ORDER)
        ...
        .map(
            item ->
                new Discipulo(
                    ...
                    item.presentes,
                    item.ausentes,
                    percentualIndividual(item.presentes, item.ausentes),
                    List.copyOf(item.evolucao)))
        .toList();
  }
  ...
  public record PainelLiderResponse(
  ...
  public record Discipulo(
      long adolescenteId,
      String nome,
      long presentes,
      long ausentes,
      BigDecimal percentualPresenca,
      List<EvolucaoIndividual> evolucao) {}

  public record EvolucaoIndividual(
      String referencia, long presentes, long ausentes, BigDecimal percentualPresenca) {}
  ...
  private static final class DiscipuloAcumulado {
    ...
    private DiscipuloAcumulado(long adolescenteId, String nome) {
