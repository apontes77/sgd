...
public class OrganizacaoController {
    ...
    @GetMapping("/discipulados") public PaginaResponse<DiscipuladoResponse> listarDiscipulados(Authentication auth, @RequestParam(required = false) Long gerenciaId, @RequestParam(required = false) Boolean ativo, @RequestParam(defaultValue = "0") @Min(0) int page, @RequestParam(defaultValue = "20") @Min(1) @Max(100) int size) { return PaginaResponse.of(discipulados.list(usuario(auth), gerenciaId, ativo, PageRequest.of(page, size)).map(DiscipuladoResponse::of)); }
    ...
    private static User usuario(Authentication auth) {
        ...
        Set<Role> perfis = auth.getAuthorities().stream()
                .map(a -> a.getAuthority()).filter(a -> a.startsWith("ROLE_"))
                .map(a -> Role.valueOf(a.substring(5))).collect(java.util.stream.Collectors.toSet());
