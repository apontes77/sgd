...
public interface DiscipuladoRepository
    extends JpaRepository<Discipulado, Long>, JpaSpecificationExecutor<Discipulado> {
  @Override
  ...
  Page<Discipulado> findAll(Specification<Discipulado> spec, Pageable pageable);
  ...
  @EntityGraph(attributePaths = {"gerencia", "discipulador", "coLideres", "coLideres.perfis"})
  @Query(
      "select distinct d from Discipulado d left join d.coLideres c where d.discipulador.id = :usuarioId or c.id = :usuarioId")
