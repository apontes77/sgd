...
public class AuthController {
  ...
  public TokenResponse login(@Valid @RequestBody LoginRequest request) {
    return TokenResponse.of(auth.login(request.email(), request.senha()));
  }
  ...
  public ResponseEntity<Void> reset(@Valid @RequestBody ResetPasswordRequest request) {
    auth.resetPassword(request.token(), request.novaSenha());
    return ResponseEntity.noContent().build();
  }

  @GetMapping("/eu")
  public UserResponse me(org.springframework.security.core.Authentication authentication) {
    return UserResponse.of((User) authentication.getPrincipal());
  }
  ...
  public record TokenResponse(String accessToken, String refreshToken, UserResponse usuario) {
    static TokenResponse of(AuthService.Tokens t) {
      return new TokenResponse(t.accessToken(), t.refreshToken(), UserResponse.of(t.user()));
    }
  }

  public record UserResponse(Long id, String nome, String email, boolean ativo, Set<Role> perfis) {
    public static UserResponse of(User user) {
      return new UserResponse(
          user.getId(), user.getNome(), user.getEmail(), user.isAtivo(), user.getPerfis());
    }
  }
}
