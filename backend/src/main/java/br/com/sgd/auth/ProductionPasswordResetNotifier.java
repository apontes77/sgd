...
public class ProductionPasswordResetNotifier implements PasswordResetNotifier {
    ...
    public ProductionPasswordResetNotifier(JavaMailSender mailSender,
                                           @Value("${app.password-reset.frontend-url:}") String frontendUrl,
                                           @Value("${app.mail.from:}") String from,
                                           SecurityProperties properties) {
        if (frontendUrl == null || frontendUrl.isBlank()) {
            throw new IllegalStateException("PASSWORD_RESET_FRONTEND_URL deve ser configurada em producao");
        }
