...
public class FrequenciaController {
  ...
  public FrequenciaController(EncontroService e, ChamadaService c) {
    encontros = e;
    chamadas = c;
  }

  @GetMapping
  public List<EncontroResponse> listar(
      @AuthenticationPrincipal User u,
      @RequestParam long discipuladoId,
      @RequestParam(required = false) LocalDate dataInicio,
      @RequestParam(required = false) LocalDate dataFim) {
    return encontros.listar(u, discipuladoId, dataInicio, dataFim).stream()
        .map(EncontroResponse::of)
        .toList();
  }

  @PostMapping
  @ResponseStatus(HttpStatus.CREATED)
  public EncontroResponse criar(
      @AuthenticationPrincipal User u, @Valid @RequestBody EncontroRequest r) {
    return EncontroResponse.of(
        encontros.criar(u, r.discipuladoId(), r.data(), r.situacao(), r.justificativa()));
  }

  @PatchMapping("/{id}")
  public EncontroResponse atualizar(
      @AuthenticationPrincipal User u,
  ...
  @PutMapping("/{id}/frequencias")
  public List<FrequenciaResponse> salvar(
      @AuthenticationPrincipal User u,
      @PathVariable long id,
      @Valid @RequestBody ChamadaRequest r) {
    return chamadas
        .salvar(
            u,
            id,
            r.frequencias().stream()
                .map(i -> new ChamadaService.ItemChamada(i.adolescenteId(), i.situacao()))
                .toList())
        .stream()
        .map(FrequenciaResponse::of)
        .toList();
  }

  @GetMapping("/{id}/visitantes")
  public VisitantesResponse consultarVisitantes(
      @AuthenticationPrincipal User u, @PathVariable long id) {
    return new VisitantesResponse(chamadas.listarVisitantes(u, id));
  }

  @PutMapping("/{id}/visitantes")
  public VisitantesResponse visitantes(
      @AuthenticationPrincipal User u,
      @PathVariable long id,
      @Valid @RequestBody VisitantesRequest r) {
    return new VisitantesResponse(chamadas.salvarVisitantes(u, id, r.quantidade()));
  }

  public record EncontroRequest(
      @NotNull Long discipuladoId,
      @NotNull LocalDate data,
      @NotNull SituacaoEncontro situacao,
  ...
  public record FrequenciaRequest(
      @NotNull Long adolescenteId, @NotNull SituacaoFrequencia situacao) {}

  public record VisitantesRequest(@Min(0) int quantidade) {}

  public record VisitantesResponse(int quantidade) {}
