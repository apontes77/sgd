...
public class ChamadaService {
  ...
  public List<Frequencia> salvar(User ator, long encontroId, List<ItemChamada> itens) {
    ...
    Map<Long, Adolescente> porId =
        atuais.stream()
            .map(v -> v.getAdolescente())
            .collect(Collectors.toMap(Adolescente::getId, Function.identity()));
    ...
    if (ids.contains(null)
        ...
        || !porId.keySet().equals(new HashSet<>(ids)))
    ...
    for (var item : itens) {
      ...
      var f =
          existente.orElseGet(
              () -> new Frequencia(e, porId.get(item.adolescenteId()), item.situacao(), agora));
      if (existente.isPresent()) f.atualizar(item.situacao(), agora);
      resultado.add(frequencias.save(f));
      if (anterior != item.situacao()) {
        var m = new LinkedHashMap<String, Object>();
        m.put("adolescenteId", item.adolescenteId());
        m.put("anterior", anterior);
        m.put("novo", item.situacao());
        mudancas.add(m);
      }
    }
    if (!mudancas.isEmpty())
