...
public class EncontroService {
 ...
 public Encontro criar(User ator,long discipuladoId,LocalDate data,SituacaoEncontro situacao,String justificativa){var d=discipulado(discipuladoId);escopo.exigirAlteracao(ator,d);if(!d.isAtivo()) conflito("O discipulado está inativo.");if(encontros.existsByDiscipuladoIdAndData(discipuladoId,data))conflito("Já existe um encontro registrado para este discipulado nesta data.");if(situacao==SituacaoEncontro.CANCELADO)exigirRegistroNaoRealizado(ator);String motivo=validarJustificativa(situacao,justificativa);var e=encontros.save(new Encontro(d,data,situacao,motivo,clock.instant()));var detalhes=new LinkedHashMap<String,Object>();detalhes.put("id",e.getId());detalhes.put("situacao",situacao);detalhes.put("justificativa",motivo);auditar(ator,"ENCONTRO","CRIAR",detalhes);return e;}
