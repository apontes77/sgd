...
public class EncontroService {
 ...
 public Encontro criar(User ator,long discipuladoId,LocalDate data,SituacaoEncontro situacao,String justificativa){var d=discipulado(discipuladoId);escopo.exigirAlteracao(ator,d);if(!d.isAtivo()) conflito("O discipulado está inativo.");if(situacao==SituacaoEncontro.CANCELADO)exigirAdministrador(ator);String motivo=validarJustificativa(situacao,justificativa);var e=encontros.save(new Encontro(d,data,situacao,motivo,clock.instant()));var detalhes=new LinkedHashMap<String,Object>();detalhes.put("id",e.getId());detalhes.put("situacao",situacao);detalhes.put("justificativa",motivo);auditar(ator,"ENCONTRO","CRIAR",detalhes);return e;}
 ...
 private static void exigirAdministrador(User ator){if(!ator.getPerfis().contains(Role.ADMIN))throw new ResponseStatusException(HttpStatus.FORBIDDEN,"Somente administradores podem marcar um encontro como não realizado.");}
