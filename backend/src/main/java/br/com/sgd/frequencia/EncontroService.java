...
public class EncontroService {
 ...
 public Encontro criar(User ator,long discipuladoId,LocalDate data,SituacaoEncontro situacao,String justificativa){var d=discipulado(discipuladoId);escopo.exigirAlteracao(ator,d);if(!d.isAtivo()) conflito("O discipulado está inativo.");if(encontros.existsByDiscipuladoIdAndData(discipuladoId,data))conflito("Já existe um encontro registrado para este discipulado nesta data.");if(situacao==SituacaoEncontro.NAO_REALIZADO)exigirRegistroNaoRealizado(ator);String motivo=validarJustificativa(situacao,justificativa);var e=encontros.save(new Encontro(d,data,situacao,motivo,clock.instant()));var detalhes=new LinkedHashMap<String,Object>();detalhes.put("id",e.getId());detalhes.put("situacao",situacao);detalhes.put("justificativa",motivo);auditar(ator,"ENCONTRO","CRIAR",detalhes);return e;}
 ...
 public Encontro atualizar(User ator,long id,LocalDate data,SituacaoEncontro situacao,String justificativa){var e=encontro(id);escopo.exigirAlteracao(ator,e.getDiscipulado());var novaData=data==null?e.getData():data;if(encontros.existsByDiscipuladoIdAndDataAndIdNot(e.getDiscipulado().getId(),novaData,id))conflito("Já existe um encontro registrado para este discipulado nesta data.");var novaSituacao=situacao==null?e.getSituacao():situacao;if(e.getSituacao()==SituacaoEncontro.NAO_REALIZADO&&novaSituacao==SituacaoEncontro.REALIZADO)exigirReversaoAdministrativa(ator);else if(e.getSituacao()==SituacaoEncontro.NAO_REALIZADO||novaSituacao==SituacaoEncontro.NAO_REALIZADO)exigirRegistroNaoRealizado(ator);String motivo=novaSituacao==SituacaoEncontro.NAO_REALIZADO?validarJustificativa(novaSituacao,justificativa==null?e.getJustificativa():justificativa):validarJustificativa(novaSituacao,justificativa);if(e.getSituacao()!=novaSituacao&&(frequencias.existsByEncontroId(id)||visitantes.findByEncontroId(id).map(v->v.getQuantidade()>0).orElse(false)))conflito("Um encontro com chamada ou visitantes registrados não pode ter sua situação alterada.");var antes=estado(e);e.atualizar(data,novaSituacao,motivo,clock.instant());auditar(ator,"ENCONTRO","ALTERAR",Map.of("id",id,"anterior",antes,"novo",estado(e)));return e;}
 ...
 private static String validarJustificativa(SituacaoEncontro situacao,String justificativa){String valor=justificativa==null?null:justificativa.trim();if(situacao==SituacaoEncontro.NAO_REALIZADO&&(valor==null||valor.isEmpty()))throw new IllegalArgumentException("A justificativa é obrigatória para encontro não realizado.");if(valor!=null&&valor.length()>500)throw new IllegalArgumentException("A justificativa deve ter no máximo 500 caracteres.");if(situacao==SituacaoEncontro.REALIZADO&&valor!=null&&!valor.isEmpty())throw new IllegalArgumentException("A justificativa só pode ser informada para encontro não realizado.");return situacao==SituacaoEncontro.NAO_REALIZADO?valor:null;}
 ...
 private static void exigirReversaoAdministrativa(User ator){if(!ator.getPerfis().contains(Role.ADMIN))throw new ResponseStatusException(HttpStatus.FORBIDDEN,"Somente administradores podem corrigir um encontro não realizado para realizado.");}
 ...
 void exigirEditavel(User ator,Encontro e){escopo.exigirAlteracao(ator,e.getDiscipulado());if(e.getSituacao()==SituacaoEncontro.NAO_REALIZADO)conflito("Não é possível registrar dados em um discipulado marcado como não realizado.");if(!ator.getPerfis().contains(br.com.sgd.user.Role.ADMIN)&&clock.instant().isAfter(e.getCriadoEm().plus(Duration.ofHours(3))))throw new ResponseStatusException(HttpStatus.FORBIDDEN,"A janela de três horas para alteração terminou.");}
