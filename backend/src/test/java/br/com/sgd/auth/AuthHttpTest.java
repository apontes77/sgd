...
class AuthHttpTest {
  ...
  void administradorConfiguradoConsegueEntrarComCredenciaisIniciais() throws Exception {
    mvc.perform(
            post("/api/v1/autenticacao/login")
                ...
                .content(
                    "{\"email\":\"bootstrap-admin@sgd.local\",\"senha\":\"senha-bootstrap-segura\"}"))
  ...
  void loginUsaCampoSenhaELogoutRevogaRefreshToken() throws Exception {
    ...
    users.saveAndFlush(
        new User("Administrador", email, passwords.encode(password), Set.of(Role.ADMIN)));
  ...
  void rejeitaTokenInvalidoAoConsultarUsuarioAtual() throws Exception {
    mvc.perform(get("/api/v1/autenticacao/eu").header("Authorization", "Bearer token-invalido"))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.status").value(401));
  }
