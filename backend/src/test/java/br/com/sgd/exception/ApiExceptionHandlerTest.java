...
class ApiExceptionHandlerTest {
    ...
    void mapsAuthenticationAndOAuthFailures() {
        assertProblem(handler.handleUnauthorized(new AuthService.InvalidCredentialsException()), 401,
                "Credenciais ou token");
        assertProblem(handler.handleUnauthorized(new AuthService.InvalidTokenException()), 401,
                "Credenciais ou token");
        assertProblem(handler.handleInvalidOAuth(new OAuthIdentityService.InvalidOAuthIdentityException()), 401,
                "identidade externa");
        assertProblem(handler.handleOAuthConflict(new OAuthIdentityService.OAuthIdentityConflictException()), 409,
                "provedor externo");
    }

    @Test
    void mapsUserAndIntegrityFailures() {
        assertProblem(handler.handleDuplicateEmail(new UserService.DuplicateEmailException()), 409, "E-mail");
        assertProblem(handler.handleUserNotFound(new UserService.UserNotFoundException()), 404, "Usu");
        assertProblem(handler.handleDataIntegrity(new DataIntegrityViolationException("constraint")), 409, "integridade");
    }
    ...
    void preservesResponseStatusReasonAndUsesDefaultTitleWhenAbsent() {
        assertProblem(handler.handleResponseStatus(new ResponseStatusException(HttpStatus.NOT_FOUND, "custom reason")),
                404, "custom reason");
        assertProblem(handler.handleResponseStatus(new ResponseStatusException(HttpStatus.GONE)), 410, "Gone");
    }

    @Test
    void unexpectedFailureDoesNotExposeExceptionDetails() {
        ResponseEntity<Map<String, Object>> response = handler.handleUnexpected(new RuntimeException("database password"));
    ...
    private void assertProblem(ResponseEntity<Map<String, Object>> response, int status, String detailFragment) {
        assertThat(response.getStatusCode().value()).isEqualTo(status);
        assertThat(response.getHeaders().getContentType().toString()).isEqualTo("application/problem+json");
        assertThat(response.getBody()).containsEntry("type", "about:blank").containsEntry("status", status);
