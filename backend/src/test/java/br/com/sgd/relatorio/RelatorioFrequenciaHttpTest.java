...
class RelatorioFrequenciaHttpTest {
  ...
  void prepararDados() {
    ...
    frequencias.saveAndFlush(
        new Frequencia(alphaPrincipal, ana, SituacaoFrequencia.AUSENTE, AGORA));
  ...
  void incluiNaoRealizadoComJustificativaNoHistoricoDoPeriodo() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/relatorios/frequencia")
                .param("dataInicio", DATA.minusDays(2).toString())
  ...
  void distingueAusenciaDeEscopoDeDataSemEncontros() throws Exception {
    consultar(token(gerenteSemEscopo))
  ...
  void consultaPeriodoDeAteDozeMesesEValidaLimites() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/relatorios/frequencia")
                ...
                .header(HttpHeaders.AUTHORIZATION, bearer(token)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.dataInicio").value(DATA.minusDays(2).toString()))
    ...
    mvc.perform(
            get("/api/v1/relatorios/frequencia")
                .param("dataInicio", DATA.toString())
                .param("dataFim", DATA.plusMonths(12).toString())
                .header(HttpHeaders.AUTHORIZATION, bearer(token)))
        .andExpect(status().isOk());
    mvc.perform(
            get("/api/v1/relatorios/frequencia")
  ...
  private Encontro encontro(Discipulado discipulado, LocalDate data, SituacaoEncontro situacao) {
    String justificativa =
        situacao == SituacaoEncontro.NAO_REALIZADO ? "Imprevisto no discipulado" : null;
