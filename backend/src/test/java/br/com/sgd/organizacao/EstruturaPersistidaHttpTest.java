...
class EstruturaPersistidaHttpTest {
  ...
  void listaUsuariosGerenciasEDiscipuladosJaPersistidos() throws Exception {
    ...
    long gerenteId =
        criarUsuario("Gerente persistido", "gerente-" + suffix + "@sgd.local", "GERENTE");
    long discipuladorId =
        criarUsuario(
            "Discipulador persistido", "discipulador-" + suffix + "@sgd.local", "DISCIPULADOR");
    long coLiderId =
        criarUsuario("Co-líder persistido", "colider-" + suffix + "@sgd.local", "CO_LIDER");
    long gerenciaId =
        criar(
            "/api/v1/gerencias",
