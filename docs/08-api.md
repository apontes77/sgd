...
- Um encontro é `REALIZADO` ou `CANCELADO`; há no máximo um encontro por discipulado/data. A não realização exige justificativa, pode ser registrada por `ADMIN` ou pelo `DISCIPULADOR` do próprio grupo e somente `ADMIN` pode revertê-la.
...
- O relatório por período reúne a união dos escopos dos perfis do usuário, aceita de um dia a 12 meses, considera somente encontros realizados e exibe apenas frequências persistidas; a impressão A4 e o salvamento em PDF usam o diálogo nativo do navegador.
