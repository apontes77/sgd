...
export interface EvolucaoDiscipulo extends IndicadorFrequencia { referencia: string }
export interface DiscipuloPainel extends Omit<IndicadorFrequencia, 'percentualPresenca'> { adolescenteId: number; nome: string; percentualPresenca: number | null; evolucao: EvolucaoDiscipulo[] }
export interface IndicadorGerencia extends IndicadorFrequencia { id: number; nome: string }
export interface IndicadorGerenciaMensal extends IndicadorFrequencia { gerenciaId: number; gerenciaNome: string; referencia: string }
export interface IndicadorSexo extends IndicadorFrequencia { sexo: 'MASCULINO' | 'FEMININO' }
export interface PainelAdminResponse { dataInicio: string; dataFim: string; resumo: ResumoPainel; evolucao: EvolucaoMensal[]; gerencias: IndicadorGerencia[]; sexos: IndicadorSexo[]; encontrosNaoRealizados: number; gerenciasMensal: IndicadorGerenciaMensal[] }
