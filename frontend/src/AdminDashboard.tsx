...
import { AnalyticsCard, LoadingState, PageHeader } from './ui'
...
export default function AdminDashboard() {
  ...
  return <Stack spacing={3}>
    <PageHeader title="Painel administrativo" description="Visão consolidada da frequência em toda a organização." eyebrow="Visão geral" />
...
function Conteudo({ dados }: { dados: PainelAdminResponse }) {
  ...
  return <Stack spacing={3}>
    ...
    <Box sx={{ display: 'grid', gap: 3, gridTemplateColumns: { xs: '1fr', xl: 'minmax(0, 1.35fr) minmax(380px, .65fr)' } }}>
      <AnalyticsCard title="Presença por gerência" description="Percentual acompanhado do volume total de registros." chart={<GraficoGerencias dados={gerencias} />} table={<TabelaGerencias dados={gerencias} ordenacao={ordenacao} onOrdenacao={setOrdenacao} />} />
      <AnalyticsCard title="Presença por sexo do discipulado" description="Comparação de percentuais e volumes." chart={<GraficoSexos dados={dados} />} table={<TabelaSexos dados={dados} />} />
    </Box>
  </Stack>
}
...
function GraficoGerencias({ dados }: { dados: IndicadorGerencia[] }) {
  return <Box role="img" aria-label="Gráfico de barras do percentual de presença por gerência."><ReactECharts style={{ height: Math.min(600, Math.max(300, dados.length * 48)) }} option={{ aria: { enabled: true }, tooltip: { trigger: 'axis', formatter: (params: Array<{ dataIndex: number; value: number }>) => { const item = dados[params[0]?.dataIndex]; return item ? `${item.nome}<br/>Presença: ${percentual(item.percentualPresenca)}<br/>Registros: ${item.presentes + item.ausentes}` : '' } }, grid: { left: 130, right: 55, bottom: 30, containLabel: true }, xAxis: { type: 'value', max: 100, axisLabel: { formatter: '{value}%' } }, yAxis: { type: 'category', data: dados.map((item) => item.nome) }, series: [{ name: 'Presença', type: 'bar', data: dados.map((item) => item.percentualPresenca), label: { show: true, position: 'right', formatter: '{c}%' }, itemStyle: { color: '#3451B2', borderRadius: [0, 5, 5, 0] } }] }} /></Box>
}
...
function GraficoSexos({ dados }: { dados: PainelAdminResponse }) {
  ...
  return <Box role="img" aria-label="Gráfico de barras comparando o percentual de presença por sexo do discipulado."><ReactECharts style={{ height: 300 }} option={{ aria: { enabled: true }, tooltip: { trigger: 'axis', formatter: (params: Array<{ dataIndex: number }>) => { const item = dados.sexos[params[0]?.dataIndex]; return item ? `${labels[params[0].dataIndex]}<br/>Presença: ${percentual(item.percentualPresenca)}<br/>Presentes: ${item.presentes}<br/>Ausentes: ${item.ausentes}` : '' } }, grid: { left: 95, right: 60, bottom: 30, containLabel: true }, xAxis: { type: 'value', min: 0, max: 100, axisLabel: { formatter: '{value}%' } }, yAxis: { type: 'category', data: labels }, series: [{ type: 'bar', data: dados.sexos.map((item) => item.percentualPresenca), label: { show: true, position: 'right', formatter: '{c}%' }, itemStyle: { color: '#0F8B8D', borderRadius: [0, 5, 5, 0] } }] }} /></Box>
}

function TabelaSexos({ dados }: { dados: PainelAdminResponse }) { return <TableContainer><Table size="small" aria-label="Resumo por sexo"><TableHead><TableRow><TableCell scope="col">Sexo</TableCell><TableCell scope="col">Presentes</TableCell><TableCell scope="col">Ausentes</TableCell><TableCell scope="col">Volume</TableCell><TableCell scope="col">Presença</TableCell></TableRow></TableHead><TableBody>{dados.sexos.map((item) => <TableRow key={item.sexo}><TableCell component="th" scope="row">{item.sexo === 'MASCULINO' ? 'Masculino' : 'Feminino'}</TableCell><TableCell>{item.presentes}</TableCell><TableCell>{item.ausentes}</TableCell><TableCell>{item.presentes + item.ausentes}</TableCell><TableCell>{percentual(item.percentualPresenca)}</TableCell></TableRow>)}</TableBody></Table></TableContainer> }
