...
describe('cliente HTTP', () => {
  ...
  it('envia o campo senha no login e armazena a sessão retornada', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
      accessToken: 'access', refreshToken: 'refresh', usuario,
    }), { status: 200, headers: { 'Content-Type': 'application/json' } }))

    await expect(authApi.login('admin@sgd.local', 'senha-segura')).resolves.toEqual(usuario)
    expect(fetchMock).toHaveBeenCalledWith('/api/v1/autenticacao/login', expect.objectContaining({
      method: 'POST', body: JSON.stringify({ email: 'admin@sgd.local', senha: 'senha-segura' }),
    }))
  ...
  it('revoga o refresh token no logout e sempre limpa a sessão local', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/v1/autenticacao/logout', expect.objectContaining({
