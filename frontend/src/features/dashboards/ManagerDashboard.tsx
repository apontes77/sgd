...
export default function ManagerDashboard() {
  ...
  const selecionado = useMemo(
    () => dados?.discipulados.find((item) => item.id === selecionadoId),
    [dados, selecionadoId],
  )
  return (
    <Stack spacing={3}>
      <PageHeader
        title="Minha gerência"
        description={dados?.gerencia.nome ?? 'Acompanhamento dos discipulados sob sua gerência.'}
      ...
      {!carregando && dados && (
        <>
          ...
          <PainelEvolucao
            titulo="Evolução mensal da gerência"
            tabelaTitulo="Resumo mensal da gerência"
            dados={normalizarMeses(dados.dataInicio, dados.dataFim, dados.evolucao)}
          />
          {dados.discipulados.length === 0 ? (
            <Alert severity="info">Não há discipulados com histórico no período.</Alert>
          ) : (
            <>
              <ResumoAtencao dados={dados.discipulados} />
              <SupervisaoNaoRealizados dados={dados.encontrosNaoRealizados} />
              <AnalyticsCard
                title="Presença por discipulado"
                description="Selecione uma barra ou uma linha para abrir o histórico detalhado."
                chart={<GraficoComparacao dados={dados.discipulados} onSelecionar={setSelecionadoId} />}
                table={<TabelaComparacao dados={dados.discipulados} onSelecionar={setSelecionadoId} />}
              />
              {selecionado && (
                <SectionCard
                  ...
                  action={
                    <FormControl sx={{ minWidth: { xs: 220, sm: 340 } }}>
                      ...
                      <Select
                        ...
                        value={selecionadoId || ''}
                        label="Discipulado"
                        onChange={(event) => setSelecionadoId(Number(event.target.value))}
                      >
                ...
                >
                  <Detalhe discipulado={selecionado} inicio={dados.dataInicio} fim={dados.dataFim} />
                </SectionCard>
              )}
            </>
          )}
...
function ResumoAtencao({ dados }: { dados: DiscipuladoPainel[] }) {
  const abaixo = dados.filter(
    (item) => item.resumo.encontrosRealizados > 0 && item.resumo.percentualPresenca < REFERENCIA_INFORMATIVA,
  ).length
  ...
  return (
    <Alert severity={abaixo || semEncontros || quedas ? 'warning' : 'info'}>
      <strong>Leitura do período:</strong> {abaixo} de {dados.length} discipulados abaixo da referência informativa de{' '}
      {REFERENCIA_INFORMATIVA}%; {semEncontros} sem encontros e {quedas} com queda no último mês disponível. Essa
      referência não representa uma meta pastoral.
    </Alert>
  )
}

function temQuedaRecente(item: DiscipuladoPainel) {
  const meses = item.evolucao.filter((mes) => mes.presentes + mes.ausentes > 0)
  return meses.length >= 2 && meses.at(-1)!.percentualPresenca < meses.at(-2)!.percentualPresenca
}

function SupervisaoNaoRealizados({ dados }: { dados: PainelGerenciaResponse['encontrosNaoRealizados'] }) {
  return (
    <SectionCard
      title={`Encontros não realizados (${dados.length})`}
          ...
          <Table size="small" aria-label="Encontros não realizados">
            <TableHead>
              <TableRow>
                ...
                <TableCell scope="col">Discipulado</TableCell>
                <TableCell scope="col">Justificativa</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {dados.map((item) => (
                <TableRow key={item.encontroId}>
      ...
      <ReactECharts
        onEvents={{
          click: (param: { dataIndex: number }) => {
            const item = ordenados[param.dataIndex]
          ...
          series: [
            {
              ...
              data: ordenados.map((item) => ({
                value: item.resumo.percentualPresenca,
                itemStyle: {
                  color:
          ...
          {dados.map((item) => (
            <TableRow key={item.id} hover>
              <TableCell component="th" scope="row">
                <Button color="inherit" onClick={() => onSelecionar(item.id)}>
                  {item.nome}
                  {!item.ativo && ' (inativo)'}
                </Button>
              </TableCell>
              <TableCell>{item.resumo.encontrosRealizados}</TableCell>
              <TableCell>{item.resumo.presentes}</TableCell>
              <TableCell>{item.resumo.ausentes}</TableCell>
              <TableCell>{percentual(item.resumo.percentualPresenca)}</TableCell>
              <TableCell>
                <Stack direction="row" spacing={0.5} flexWrap="wrap">
                  {item.resumo.encontrosRealizados === 0 && <Chip size="small" label="Sem encontros" />}
                  {temQuedaRecente(item) && <Chip size="small" color="warning" label="Queda recente" />}
                </Stack>
              </TableCell>
            </TableRow>
          ))}
...
function Detalhe({ discipulado, inicio, fim }: { discipulado: DiscipuladoPainel; inicio: string; fim: string }) {
  return (
    <Stack spacing={2}>
      <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
        <Typography variant="h5">{discipulado.nome}</Typography>
        {!discipulado.ativo && <Chip color="warning" label="Inativo" size="small" />}
        {temQuedaRecente(discipulado) && (
          <Chip color="warning" variant="outlined" label="Queda no último mês disponível" size="small" />
        )}
      </Stack>
      <KpisPresenca resumo={discipulado.resumo} />
      <PainelEvolucao
        titulo="Evolução mensal do discipulado"
        tabelaTitulo="Histórico mensal do discipulado"
        dados={normalizarMeses(inicio, fim, discipulado.evolucao)}
      />
    </Stack>
  )
}
