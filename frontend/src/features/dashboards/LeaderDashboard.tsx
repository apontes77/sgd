...
export default function LeaderDashboard() {
  ...
  useEffect(() => {
    ...
    setErro('')
    painelApi
      .consultarLider(periodo.inicio, periodo.fim)
      .then((resposta) => {
        if (ativo) {
          setDados(resposta)
          setDiscipuloId((atual) =>
            resposta.discipulos.some((item) => item.adolescenteId === atual)
              ? atual
              : resposta.discipulos[0]?.adolescenteId,
          )
        }
      })
      .catch((error: Error) => {
  ...
  return (
    <Stack spacing={3}>
      <PageHeader
        ...
        description={dados?.discipulado.nome ?? 'Histórico de presença do grupo em que você exerce liderança.'}
        eyebrow="Visão da liderança"
        action={dados && !dados.discipulado.ativo ? <Chip color="warning" label="Inativo" size="small" /> : undefined}
      />
      <FiltroPeriodo
      ...
      {!carregando && dados && (
        <>
          ...
          <PainelEvolucao
            ...
            dados={meses}
          />
          <PainelIndividual dados={dados} selecionado={discipulo} onSelecionar={setDiscipuloId} />
        </>
      )}
    </Stack>
  )
...
function PainelIndividual({
  dados,
  selecionado,
  onSelecionar,
}: {
  dados: PainelLiderResponse
  selecionado?: DiscipuloPainel
  onSelecionar: (id: number | undefined) => void
}) {
  if (dados.discipulos.length === 0)
    return (
      <SectionCard title="Progresso individual de presença">
        <EmptyState
          title="Nenhum discípulo no período"
          description="Não há discípulos vinculados ou com frequência registrada no intervalo selecionado."
        />
      </SectionCard>
    )
  ...
  return (
    <Stack spacing={2}>
      <SectionCard
        title="Progresso individual de presença"
        description="Pesquise e selecione um discípulo para acompanhar sua presença no intervalo aplicado."
      >
        <Autocomplete
          fullWidth
          options={dados.discipulos}
          value={selecionado ?? null}
          onChange={(_, valor) => onSelecionar(valor?.adolescenteId)}
          getOptionLabel={(item) => item.nome}
          isOptionEqualToValue={(option, value) => option.adolescenteId === value.adolescenteId}
          renderInput={(params) => <TextField {...params} label="Discípulo" placeholder="Pesquisar discípulo" />}
          noOptionsText="Nenhum discípulo encontrado"
        />
      </SectionCard>
      {selecionado && (
        <>
          <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', sm: 'repeat(3, minmax(0, 1fr))' } }}>
            <KpiCard label="Presentes" value={selecionado.presentes} icon={<PersonRounded />} tone="success" />
            <KpiCard label="Ausentes" value={selecionado.ausentes} icon={<PersonOffRounded />} tone="error" />
            <KpiCard
              label="Presença"
              value={selecionado.percentualPresenca == null ? '—' : percentual(selecionado.percentualPresenca)}
              icon={<HowToRegRounded />}
              tone="primary"
            />
          </Box>
          {selecionado.evolucao.length === 0 && (
            <Alert severity="info">
              Não há registros de frequência para {selecionado.nome} no período selecionado.
            </Alert>
          )}
          <AnalyticsCard
            title={`Presença mensal de ${selecionado.nome}`}
            description="Percentual calculado apenas com registros de presença e ausência do discípulo."
            chart={<GraficoIndividual nome={selecionado.nome} dados={meses} />}
            table={<TabelaIndividual nome={selecionado.nome} dados={meses} />}
          />
        </>
      )}
    </Stack>
...
function normalizarDiscipulo(inicio: string, fim: string, evolucao: DiscipuloPainel['evolucao']): MesDiscipulo[] {
  const base = normalizarMeses(
    inicio,
    fim,
    evolucao.map((item) => ({ ...item, visitantes: 0 })),
  )
  ...
  return base.map((item) => ({
    ...
    possuiRegistro: referencias.has(item.referencia),
  }))
}
...
function TabelaIndividual({ nome, dados }: { nome: string; dados: MesDiscipulo[] }) {
  return (
    <TableContainer>
      <Table size="small" aria-label={`Histórico mensal de presença de ${nome}`}>
        <TableHead>
          <TableRow>
