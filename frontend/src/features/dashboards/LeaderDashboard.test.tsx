...
const resposta = {
  ...
  dataFim: '2026-03-31',
  discipulado: { id: 1, nome: 'Discipulado Esperança', sexo: 'FEMININO', ativo: true },
  resumo: { encontrosRealizados: 1, presentes: 3, ausentes: 1, visitantes: 2, percentualPresenca: 75 },
  evolucao: [{ referencia: '2026-02', presentes: 3, ausentes: 1, visitantes: 2, percentualPresenca: 75 }],
  discipulos: [
    {
      adolescenteId: 10,
      nome: 'Ana Clara',
      presentes: 2,
      ausentes: 1,
      percentualPresenca: 66.67,
      evolucao: [{ referencia: '2026-02', presentes: 2, ausentes: 1, percentualPresenca: 66.67 }],
    },
    { adolescenteId: 11, nome: 'Beatriz', presentes: 0, ausentes: 0, percentualPresenca: null, evolucao: [] },
  ],
}
...
describe('painel do líder', () => {
  ...
  it('mostra KPIs, série contínua e tabela acessível do próprio grupo', async () => {
    ...
    expect(
      screen.getByRole('img', { name: 'Gráfico do percentual mensal de presença de Ana Clara.' }),
    ).toBeInTheDocument()
  ...
  it('apresenta a mensagem de conflito devolvida pela API', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ detail: 'O usuário possui mais de uma associação de liderança.' }), {
        status: 409,
  ...
  it('mostra estado vazio quando não há discípulos no período', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ ...resposta, discipulos: [] }), {
        status: 200,
