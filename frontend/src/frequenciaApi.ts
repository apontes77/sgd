...
export const frequenciaApi = {
  ...
  criarEncontro: (body:{discipuladoId:number;data:string;situacao:SituacaoEncontro;justificativa?:string}) => request<Encontro>('/encontros',{method:'POST',body:JSON.stringify(body)}),
  atualizarEncontro: (id:number, body:{data?:string;situacao?:SituacaoEncontro;justificativa?:string}) => request<Encontro>(`/encontros/${id}`,{method:'PATCH',body:JSON.stringify(body)}),
