    ...
    >
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems={{ sm: 'center' }}>
        ...
        <TextField
          ...
          type="date"
          value={dataFim}
          onChange={(event) => onFim(event.target.value)}
          slotProps={{ inputLabel: { shrink: true } }}
          error={Boolean(dataInicio && dataFim && dataInicio > dataFim)}
          helperText={
...
export function KpisPresenca({ resumo }: { resumo: ResumoPainel }) {
  const itens = [
    ...
    {
      nome: 'Presença',
      valor: percentual(resumo.percentualPresenca),
  ...
  return (
    ...
    >
      {itens.map((item) => (
        <KpiCard key={item.nome} label={item.nome} value={item.valor} icon={item.icon} tone={item.tone} />
      ))}
    </Box>
  )
}

export function PainelEvolucao({
  titulo,
  tabelaTitulo,
  dados,
}: {
  ...
  return (
    <AnalyticsCard
      title={titulo}
      description="Volumes no eixo esquerdo e presença no eixo direito. Visitantes não entram no cálculo da presença."
      chart={<GraficoEvolucao titulo={titulo} dados={dados} />}
      table={<TabelaEvolucao titulo={tabelaTitulo} dados={dados} />}
    />
  )
}
...
export function GraficoEvolucao({ titulo, dados }: { titulo: string; dados: MesVisual[] }) {
  ...
  const valor = (
    item: MesVisual,
    campo: keyof Pick<EvolucaoMensal, 'presentes' | 'ausentes' | 'visitantes' | 'percentualPresenca'>,
  ) => (item.possuiEncontro ? item[campo] : null)
  return (
    <Box
      role="img"
      aria-label={`${titulo}. Gráfico combinado de presentes, ausentes, visitantes e percentual de presença por mês.`}
      ...
      <Table size="small" aria-label={titulo}>
        ...
        <TableHead>
          <TableRow>
            ...
            <TableCell scope="col">Visitantes</TableCell>
