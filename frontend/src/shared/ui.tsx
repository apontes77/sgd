import { BarChartRounded, CloseRounded, TableRowsRounded } from '@mui/icons-material'
...
export function SectionCard({
  ...
  return (
    <Paper variant="outlined" sx={{ overflow: 'hidden', boxShadow: '0 1px 3px rgba(23,32,51,.04)', ...sx }}>
      {(title || action) && (
        <Stack
          direction="row"
          justifyContent="space-between"
...
export function KpiCard({
  ...
  return (
    <Paper
      variant="outlined"
      sx={{ p: 2.25, minWidth: 0, position: 'relative', overflow: 'hidden', boxShadow: '0 1px 3px rgba(23,32,51,.04)' }}
    >
...
export function FilterToolbar({
  ...
  return (
    <Paper
      ...
      onSubmit={onSubmit}
      variant="outlined"
      sx={{ p: 2, bgcolor: '#FBFCFE', boxShadow: 'none' }}
    >
      {children}
    </Paper>
  )
...
export function DataTableCard({ children, sx }: { children: ReactNode; sx?: SxProps<Theme> }) {
  return (
    <Paper variant="outlined" sx={{ overflowX: 'auto', boxShadow: '0 1px 3px rgba(23,32,51,.04)', ...sx }}>
      {children}
    </Paper>
  )
}
