import type { EvolucaoMensal } from '@/features/dashboards/api'

export interface MesVisual extends EvolucaoMensal {
...
export function normalizarMeses(inicio: string, fim: string, dados: EvolucaoMensal[]): MesVisual[] {
  ...
  while (cursor <= limite) {
    ...
    meses.push(
      item
        ? { ...item, possuiEncontro: true }
        : { referencia, presentes: 0, ausentes: 0, visitantes: 0, percentualPresenca: 0, possuiEncontro: false },
    )
