...
export interface UserManagementClient {
  ...
  create(body: { nome: string; email: string; perfis: Perfil[] }): Promise<Usuario>
  update(id: number, body: { nome?: string; perfis?: Perfil[]; ativo?: boolean }): Promise<Usuario>
  resendSetup?(id: number): Promise<void>
}
...
const emptyForm = { nome: '', email: '', perfis: [] as Perfil[] }
...
export default function UserManagement({ client }: { client: UserManagementClient }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await client.create(form); setForm(emptyForm); setDrawerOpen(false); setMessage('Usuário criado. Enviamos um convite para definição de senha.'); await load(0)
  ...
  async function resendSetup(user: Usuario) {
    if (!client.resendSetup) return
    setError(''); setMessage(''); setLoading(true)
    try {
      await client.resendSetup(user.id)
      setMessage(`Convite de definição de senha reenviado para ${user.email}.`)
    } catch (reason) {
  ...
  function statusLabel(user: Usuario) {
    ...
    if (!user.senhaDefinida) return 'Aguardando definição de senha'
    return 'Ativo'
  }
  ...
  return <Stack spacing={3}>
    ...
    <DataTableCard>{loading && <LinearProgress />}<Table><TableHead><TableRow><TableCell>Nome</TableCell><TableCell>E-mail</TableCell><TableCell>Perfis</TableCell><TableCell>Status</TableCell><TableCell align="right">Ações</TableCell></TableRow></TableHead>
      <TableBody>{result.content.map((user) => <TableRow key={user.id} hover>
        <TableCell><Typography variant="body2" fontWeight={650}>{user.nome}</Typography></TableCell>
        <TableCell>{user.email}</TableCell>
        <TableCell>{user.perfis.map((role) => <Chip key={role} label={roleLabel[role]} size="small" variant="outlined" sx={{ mr: 0.5, my: 0.25 }} />)}</TableCell>
        <TableCell>{user.senhaDefinida === false && user.ativo !== false ? <Chip size="small" color="warning" label={statusLabel(user)} /> : <StatusChip active={Boolean(user.ativo)} />}</TableCell>
        <TableCell align="right">
          <Stack direction="row" spacing={1} justifyContent="flex-end">
            {user.ativo !== false && !user.senhaDefinida && client.resendSetup && (
              <Button disabled={loading} onClick={() => void resendSetup(user)}>Reenviar convite</Button>
            )}
            <Button color={user.ativo ? 'warning' : 'primary'} onClick={() => setPendingUser(user)}>{user.ativo ? 'Inativar' : 'Reativar'}</Button>
          </Stack>
        </TableCell>
      </TableRow>)}</TableBody>
    ...
    >
      ...
      <Typography variant="body2" color="text.secondary">O usuário receberá um e-mail para definir a própria senha.</Typography>
      <Box>
