...
export default function UserManagement({ client }: { client: UserManagementClient }) {
  ...
  const [form, setForm] = useState({ nome: '', email: '', perfis: [] as Perfil[] })
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await client.create(form); setForm({ nome: '', email: '', perfis: [] }); setMessage('Usuário criado. Enviamos um convite para definição de senha.'); await load(0)
