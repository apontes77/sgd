#!/usr/bin/env python3
"""Carga idempotente da estrutura organizacional (gerentes + discipuladores) via API admin.

Lotes: masculino e feminino. Lê CSVs em scripts/data/, valida, dry-run ou aplica.
...
# Aliases extras quando a coluna GERÊNCIA do CSV não bate com o nome do gerente
EXTRA_GERENCIA_ALIASES: dict[str, dict[str, str]] = {
    "masculino": {
        "pastor leo": "leodokmos@gmail.com",
    },
    "feminino": {
        "pastora andressa": "drealvares@gmail.com",
    },
}
...
LOTES: dict[str, dict[str, Any]] = {
    "masculino": {
        "sexo": "MASCULINO",
        "gerentes": DATA_DIR / "gerentes_masculino.csv",
        "discipulados": DATA_DIR / "discipulados_masculino.csv",
    },
    "feminino": {
        "sexo": "FEMININO",
        "gerentes": DATA_DIR / "gerentes_feminino.csv",
        "discipulados": DATA_DIR / "discipulados_feminino.csv",
    },
}
...
def normalize_email(value: str) -> str:
    # Remove quebras/espaços internos (CSV feminino traz e-mails multilinha)
...
def build_gerencia_aliases(lote: str, gerentes: list[GerenteRow]) -> dict[str, str]:
    aliases = {fold(g.nome): g.email for g in gerentes}
    aliases.update(EXTRA_GERENCIA_ALIASES.get(lote, {}))
    return aliases


def build_plan(lote: str, sexo: str, gerentes_path: Path, discipulados_path: Path) -> CargaPlan:
    gerentes = load_gerentes(gerentes_path)
    discipulados = load_discipulados(discipulados_path)
    plan = CargaPlan(lote=lote, sexo=sexo, gerentes=gerentes, discipulados=discipulados)
    aliases = build_gerencia_aliases(lote, gerentes)
    ...
    for email, gerente in list(gerente_by_email.items()):
        ...
        try:
            mapped = map_sexo(gerente.sexo)
            if mapped != sexo:
                plan.errors.append(
                    f"gerente {gerente.nome}: sexo {mapped} diverge do lote {sexo}"
                )
    ...
    for row in discipulados:
        ...
        if not row.email or not EMAIL_RE.match(row.email):
            plan.errors.append(
                f"discipulador com e-mail inválido: {row.discipulador_nome!r} / {row.email!r}"
            )
        try:
            map_faixa(row.faixa)
        except CargaError as error:
...
def print_preflight(plan: CargaPlan) -> None:
    print(f"=== Pré-voo ({plan.lote} / {plan.sexo}) ===")
...
def merge_perfis(existing: list[str] | None, desired: list[str]) -> list[str]:
    return sorted(set(existing or []) | set(desired))
    ...
    *,
    ...
    for email, expected in sorted(plan.users.items()):
        ...
        if current is None:
            ...
            merged = merge_perfis(current.get("perfis"), body_perfis)
            if dry_run or client is None:
                print(f"[dry-run] atualizar usuário {expected['nome']} <{email}> perfis={merged}")
                result[email] = current
                continue
            current = client.request(
                "PATCH",
                f"/usuarios/{current['id']}",
                {"nome": expected["nome"], "perfis": merged, "ativo": True},
            )
            print(f"[atualizado] usuário {expected['nome']} <{email}> perfis={merged}")
        result[email] = current
    return result
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Carga de gerentes e discipuladores (lotes masculino/feminino) via API."
    )
    parser.add_argument(
        "--lote",
        choices=sorted(LOTES),
        default="masculino",
        help="lote a carregar (padrão: masculino)",
    )
    ...
    parser.add_argument("--gerentes-csv", type=Path, default=None)
    parser.add_argument("--discipulados-csv", type=Path, default=None)
    parser.add_argument("--dry-run", action="store_true", help="valida sem mutar o banco")
    parser.add_argument(
        "--validate-only",
        action="store_true",
    ...
    lote_cfg = LOTES[args.lote]
    gerentes_csv = args.gerentes_csv or lote_cfg["gerentes"]
    discipulados_csv = args.discipulados_csv or lote_cfg["discipulados"]
    sexo = lote_cfg["sexo"]

    if not gerentes_csv.exists() or not discipulados_csv.exists():
        raise CargaError(f"CSVs do lote {args.lote} não encontrados em scripts/data/")

    plan = build_plan(args.lote, sexo, gerentes_csv, discipulados_csv)
    ...
    if new_passwords:
        ...
        if out is None:
            ...
            out = Path(f"/tmp/sgd-carga-senhas-{args.lote}-{stamp}.csv")
        subset = CargaPlan(
            lote=plan.lote,
            sexo=plan.sexo,
    ...
    print(f"Carga do lote {args.lote} concluída.")
    return 0
