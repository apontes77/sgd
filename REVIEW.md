...
O job de backend executa `mvn clean verify` com Java 21 (incluindo Spotless, PMD, SpotBugs e JaCoCo). O job de frontend instala com lockfile congelado e executa `test`, `lint`, `format:check` e `build` com Node 22 e pnpm 11.7.0.

### DEC-025 — Commits devem ser semânticos e separados por responsabilidade

**Status:** Implementada.

Mudanças independentes devem formar commits coerentes de `feat`, `fix`, `test` ou `docs`, evitando misturar autenticação, painel, relatório e infraestrutura no mesmo commit. O hook `commit-msg` com commitlint convencional valida o formato das mensagens; o pre-commit com lint-staged formata e lint o código staged.
...
**Status:** Implementada (parcialmente). Substituída em escopo por DEC-030.

Foi proposta a adoção de JaCoCo com limites progressivos de linhas e ramificações. O `pom.xml` agora configura `jacoco-maven-plugin` com `jacoco:check` na fase `verify`, com limiares iniciais de 70% de linhas e 50% de ramificações. A cobertura deve continuar priorizando autenticação, frequência, escopo organizacional e consultas PostgreSQL; os limiares podem ser elevados progressivamente.
...
### DEC-030 — Qualidade de código, modularidade e linting

**Status:** Implementada.

O monorepo adotou gates de qualidade e uma organização modular alinhada aos domínios de negócio:

**Backend**

- Spotless com Google Java Format, remoção de imports não usados e ordenação de imports, ligado a `verify`.
- PMD e SpotBugs com rulesets/exclusões calibrados, ligados a `verify`.
- JaCoCo com limiares iniciais de cobertura (ver DEC-028).
- `OrganizacaoController` passou para `br.com.sgd.organizacao` e `PaginaResponse` para `br.com.sgd.common`, eliminando o pacote genérico `controller`.

**Frontend**

- Prettier e ESLint reforçado (`jsx-a11y`, ordenação de imports, `consistent-type-imports`).
- Alias `@/*` e reorganização em `app/`, `shared/` e `features/<domínio>/`.
- CI executa `test`, `lint`, `format:check` e `build`.

**Repositório**

- Husky + lint-staged no pre-commit e commitlint convencional no commit-msg, operacionalizando a DEC-025.
...
Itens previamente divergentes e já alinhados por esta decisão:

- `docs/07-arquitetura.md` passa a descrever o pacote-por-domínio, a estrutura do frontend e o relatório HTML/impressão nativa;
- o workflow de frontend inclui `test`, `lint`, `format:check` e `build`.

Até que o `README.md` seja atualizado, o código, o contrato OpenAPI e as decisões deste arquivo são a referência para o comportamento já entregue.
